...
class Engine:
    ...
    def LoadScripts(folder="Assets"):
        ...
        # --- garder l'ancien contenu pour éviter de casser du code
        old_lines = []
        if os.path.exists(init_file):
            with open(init_file, "r") as f:
                old_lines = f.readlines()

        # --- regénérer les imports
        with open(init_file, "w") as f:
            f.write("# Auto-generated by UnipyEngine\n")
            for file in os.listdir(folder):
                if file.endswith(".py") and file != "__init__.py":
                    module_name = os.path.splitext(file)[0]
                    line = f"from .{module_name} import *\n"
                    if line not in old_lines:  # évite les doublons
                        f.write(line)

        # --- importer réellement les modules
        for file in os.listdir(folder):
            if file.endswith(".py") and file != "__init__.py":
